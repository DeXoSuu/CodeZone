document.getElementById('myForm').addEventListener('submit', function(event) {
    ...
    // Étape 1 : Obtenir l'adresse IP publique de l'utilisateur
    fetch('https://api.ipify.org?format=json')
        .then(response => response.json())
        .then(ipData => {
            const userIP = ipData.ip;

            // Étape 2 : Récupérer les données de géolocalisation avec l'IP de l'utilisateur
        ...
        .catch(error => {
            console.error('Erreur IPify:', error);
        });
});
